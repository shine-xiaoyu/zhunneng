...
public class HLUtils {
	...
	public static PageInfo<?> findByHighLight(ElasticsearchTemplate elasticsearchTemplate, Class<?> clazz, Integer page,
		...
		//elasticsearchTemplate ：高亮操作的模板对象
		//clazz :指定要操作的实体类字节码
		//page ：当前页
		//rows ：每页显示条数
		//fieldNames ：制定搜索字段的数组 ：new String[]{"title","name"}
		//sortField :指定排序的字段，通常为id
		//value : 搜索的关键字
		//
							...
							if (searchHits != null && searchHits.length > 0) {
								...
								for (int i = 0; i < searchHits.length; i++) {
									...
									for (int k = 0; k < fields.length; k++) {
										...
										//如果一个实体类中引用了另外的一个实体类，想要实现高亮操作，就在下边的判断中加上!fieldName.equals("实体类的属性名")
